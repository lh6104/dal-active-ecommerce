require([
    ...
    function bindDetailAccordions($scope) {
        $scope.on('click keydown', '.dalactive-product-accordion-trigger', function (event) {
            ...
            $trigger.find('.accordion-icon').text(expanded ? '+' : '-');
            $panel.prop('hidden', expanded);
            window.setTimeout(function () {
                $(window).trigger('dalactive:product-detail-recalc');
            }, 30);
        });
    }

    function bindPinnedProductScroll() {
        var section = document.querySelector('.product-detail-scroll-section'),
            infoInner = document.querySelector('.product-info-column > .product-info-main');
