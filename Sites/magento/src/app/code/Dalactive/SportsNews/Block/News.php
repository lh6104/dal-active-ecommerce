...
namespace Dalactive\SportsNews\Block;
...
{
    private const CACHE_KEY = 'dalactive_sports_news_rss_v1';
    private const CACHE_TAG = 'dalactive_sports_news';
    ...
    {
        $rssUrl = $this->scopeConfig->getValue('sportsnews/rss/rss_feed_url');
        if (!$rssUrl) {
        ...
        try {
            ...
            $this->curl->setHeaders([
                'User-Agent' => 'DalactiveSportsNews/1.0 (https://dalactive.test)'
            ]);
            ...
            $this->logger->error('SportsNews Module Error: ' . $e->getMessage());
        }
    ...
    private function getCachedNewsItems()
    {
        ...
        try {
            ...
            $this->logger->error('SportsNews cache decode error: ' . $e->getMessage());
            return [];
        }
    }
    ...
    private function getHtmlNewsItems(): array
    {
        ...
        foreach (self::HTML_FALLBACK_URLS as $url) {
            try {
                ...
                $this->logger->error('SportsNews HTML fallback error: ' . $exception->getMessage());
                continue;
            }
    ...
    private function getArticleImageFromPage(string $url): string
    {
        ...
        try {
            ...
            $this->logger->error('SportsNews article image error: ' . $exception->getMessage());
            return '';
        }
    ...
    public function getPaginatedEconomicNews()
    {
        ...
        $pageSize = (int) $this->scopeConfig->getValue('sportsnews/rss/items_per_page');
        if (!$pageSize) {
    ...
    {
        return $this->getUrl('sportsnews/index/index');
    }
